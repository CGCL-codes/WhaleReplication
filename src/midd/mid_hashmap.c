...
void defaultPut(HashMap* hashMap, void* key, void* value) {
	...
	if (hashMap->hash_list[index].key == NULL) {
		...
		// INFO_LOG("put key %s, value %s", (char*)key, (char*)value);
	}
	else {
	...
	if (hashMap->size >= hashMap->listSize && hashMap->changeSize) {
		...
		// INFO_LOG("hashMap->listSize resize to %d", hashMap->listSize);
	}
}
...
void resetHashMap(HashMap* hashMap, int new_listSize) {
	...
	// INFO_LOG("hashMap->size is %d", hashMap->size);
	...
	for(; i < hashMap->listSize; i++){
		...
		while(cnt){
			if(cnt->key == NULL){
				// INFO_LOG("%d is miss",i );
				break;
			}
			tempList[count].key = cnt->key;
			tempList[count].value = cnt->value;
			tempList[count].next = NULL;
			// INFO_LOG("Reput key %s, value %s", tempList[count].key, tempList[count].value);
			Entry * cnt_tp = cnt;
			cnt = cnt->next;
			if(j !=0)
				free(cnt_tp);
	...
	// INFO_LOG("loop 1 down! count = %d", count);
	...
	// INFO_LOG("初始化数据ok! , hashMap->size = %d ", hashMap->size);
